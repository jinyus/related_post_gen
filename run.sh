...
run_julia() {
    echo "Running Julia" &&
        cd ./julia &&
        julia --project=Related -e 'using Pkg; Pkg.instantiate()' &&
        if [ $HYPER == 1 ]; then
            capture "Julia" hyperfine -r 10 -w 5 --show-output "julia --project=Related related.jl"
        else
            command ${time} -f '%es %Mk' julia --project=Related related.jl
        fi
