use std::{ collections::BinaryHeap, ffi::CString, slice, str };
use rustc_hash::FxHashMap; // Changed this line
...
#[no_mangle]
pub extern "C" fn get_related_posts(posts_json: *const u8, len: usize) -> *mut i8 {
    let slice = unsafe { slice::from_raw_parts(posts_json, len) };

    let posts: Vec<Post> = from_slice(slice).unwrap();
...
#[no_mangle]
pub extern "C" fn free_related_posts(ptr: *mut i8) {
    unsafe {
        if !ptr.is_null() {
